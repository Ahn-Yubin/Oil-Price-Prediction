...
class DeepLstmTcnFusion(nn.Module):
    def __init__(
        self,
        *,
        ...
        self.fusion_gate = MLP(hidden_dim * 2 + context_dim, hidden_dim, hidden_dim, dropout=dropout)
        self.context_film = nn.Linear(context_dim, hidden_dim * 2)
        head_in = hidden_dim
        self.quantile_head = MLP(head_in, hidden_dim, self.horizon * len(QUANTILE_LEVELS), dropout=dropout)
        self.direction_head = MLP(head_in, hidden_dim, self.horizon, dropout=dropout)
        self.volatility_head = MLP(head_in, hidden_dim, self.horizon, dropout=dropout)
        self.confidence_head = MLP(head_in + context_dim, hidden_dim, 1, dropout=dropout)
    ...
    def forward(
        ...
        gate = torch.sigmoid(self.fusion_gate(torch.cat([lstm_repr, tcn_repr, context], dim=-1)))
        fused = gate * lstm_repr + (1.0 - gate) * tcn_repr
        gamma_beta = self.context_film(context)
        gamma, beta = torch.chunk(gamma_beta, 2, dim=-1)
        fused = fused * (1.0 + torch.tanh(gamma)) + beta
        quantiles = self.quantile_head(fused).view(batch, self.horizon, len(QUANTILE_LEVELS))
        quantiles = enforce_quantile_monotonicity(quantiles)
        prob_up = torch.sigmoid(self.direction_head(fused))
        expected_volatility = F.softplus(self.volatility_head(fused)) + 1e-6
        confidence = torch.sigmoid(self.confidence_head(torch.cat([fused, context], dim=-1)))
        return {
            "quantiles": quantiles,
            "prob_up": prob_up,
