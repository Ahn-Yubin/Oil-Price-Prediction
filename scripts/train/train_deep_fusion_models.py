...
def build_dataset(args: argparse.Namespace, symbols: list[str], config: DeepDatasetConfig):
    if args.synthetic or args.quick_test:
        return build_synthetic_deep_dataset(config), {"source": "synthetic", "symbols_used": symbols, "warnings": []}
    samples = []
    warnings: list[str] = []
    for symbol in symbols:
        try:
            ...
            if frame is None:
                ...
                continue
            ds = build_deep_dataset_from_frame(symbol=symbol, interval=args.interval, candles=frame, config=config)
            samples.extend(ds.samples)
        except Exception as exc:
            warnings.append(f"{symbol}: {exc}")
    if not samples:
        fallback = build_synthetic_deep_dataset(config)
        return fallback, {"source": "synthetic_fallback", "symbols_used": symbols, "warnings": warnings}
...
def train_one(model_name: str, args: argparse.Namespace, dataset, config: DeepDatasetConfig, data_report: dict[str, Any]) -> dict[str, Any]:
    ...
    metadata: dict[str, Any] = {
        ...
        "training_cutoff": max((sample.as_of_time.isoformat() for sample in dataset.samples), default=None),
        "metrics": {},
        "data_report": data_report,
        "status": "metadata_only" if args.metadata_only else "available",
    }
