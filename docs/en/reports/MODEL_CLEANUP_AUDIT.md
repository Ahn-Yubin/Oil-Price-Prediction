...
Standalone `cycle`, live cached `lstm`/`tcn`, and fixed-weight `ensemble` are removed from default API responses, the frontend selector, and the default backtest list. Cycle information moves into deep price features: `cycle_strength`, `cycle_phase_sin`, and `cycle_phase_cos`. LSTM and TCN are retained only as encoders inside `deep_lstm_tcn_fusion` and `llm_context_seq_moe`.
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| motif | `market_ai/modeling/forecasters/motif.py` | historical analogue | yes | yes | yes | no | partial | keep | deep primary when available | explainability and analogue panel value | short history can fall back | keep and document |
...
- API imports: `forecast_model_comparison` now contributes only `motif` and `pattern_mlp`; baselines and deep models are composed in the service layer.
- Frontend: removed models are absent from the selector, which now sends `/api/forecast?models=...`.
- Backtesting: removed models are absent from `FORECASTERS`; explicit requests return a clear error.
- Artifacts: the registry scans `.npz` for `pattern_mlp` and `.pt` for deep models.
